...
## Loading count data
The count data was downloaded and 32 samples (from 16 individduals) were selected.
...
genders <- factor(annot$Characteristics.sex.)
ages <- annot$Characteristics.age.
```

Gender, age, and the measured Vit D serum levels (before and after treatment) are possible confounding factors that can be added to the model. We chose not to add measured Vit D serum levels to reduce complexity.
...
for (row in 1:nrow(annot)) {
  ...
  filePath <- annot[row,]$Derived.Array.Data.File
  if (treatment == "none") {
    treatment <- "0"
  } else {
    treatment <- "1"
  }
  colName <- paste(treatment, indiv, sep = "_")
  counts <- read.table(paste("./data/RNAseq/txts/", filePath, sep = ""), sep = "\t", header = FALSE)$V2
  df[,colName] <- counts
}

# name rownames (= gene names)
rowNames <- read.table("./data/RNAseq/txts/AD006S6_S5.txt", sep = "\t", header = FALSE)$V1
...
Check for and remove diplicate rows.
...
Distances on an MDS plot of a DGEList object correspond to leading log-fold-change between each pair of samples. Leading log-fold-change is the root-mean-square average of the largest log2-fold-changes between each pair of samples. Each pair of samples extracted from each patient tend to cluster together, suggesting a batch effect. The Vit D treated samples tend to be above the non-treated samples for each patient, suggesting a treatment effect within paired sample. 
...
```{r}
# Create a design matrix that includes the treatment and confounding factors
design <- model.matrix(~ treatments + individuals + genders + ages)
...
result <- topTags(lrt, n = 500)
result <- result[result[["table"]][["FDR"]] < 0.05,]
result
```

After FDR-correction, 75 significantly differentially expressed genes could be determined.
